...
# @register(pattern=r".dwl(?: |$)(.*)", outgoing=True)


@borg.on(admin_cmd(pattern="dwl(?: |$)(.*)", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
...
# @borg.on(admin_cmd(pattern="upl (.*)", allow_sudo=True)
...
CMD_HELP.update(
    {
        "download": ".dwl <link|filename> or reply to media\
