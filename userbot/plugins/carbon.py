...
async def carbon_api(e):
 ...
 if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
   ...
   sleep(0.001) # this might take a bit.
   #driver.find_element_by_xpath("//button[contains(text(),'4x')]").click()
   #sleep(0.001)
   await e.edit("⬛⬛⬛⬜⬜")
   #driver.find_element_by_xpath("//button[contains(text(),'PNG')]").click()
   #sleep(0.001) #Waiting for downloading
   ...
   await e.client.send_file(
         e.chat_id,
         file,
         caption="cαʀɮօռɨֆɛɖ ɮʏ[мɛɢαֆтαʀ](https://github.com/Bristi-OP/MEGASTAR)",
...
CMD_HELP.update(
    {
        "carbonRBG": 
    ".karb <reply to text> "
    "\nCreate a cool random colorful carbon image for that text (doesn't work in media)"
    })
