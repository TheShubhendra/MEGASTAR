...
from userbot.tweethelp import moditweet, johnnytweet, sunnytweet, bhautweet, jtweet, miatweet
...
async def trumptweet(text):
    r = requests.get(
        f"https://nekobot.xyz/api/imagegen?typ=trumptweet&text={text}"
    ).json()
    hola = r.get("message")
    pglet = url(hola)
    if not pglet:
    ...
    with open("gpx.png", "wb") as f:
        f.write(requests.get(hola).content)
...
async def changemymind(text):
    r = requests.get(
        f"https://nekobot.xyz/api/imagegen?typ=changemymind&text={text}"
    ).json()
    hola = r.get("message")
    pglet = url(hola)
    if not pglet:
...
async def tweets(text1, text2):
    r = requests.get(
        f"https://nekobot.xyz/api/imagegen?typ=tweet&text={text1}&username={text2}"
    ).json()
    hola = r.get("message")
    pglet = url(hola)
    if not pglet:
...
@borg.on(admin_cmd(outgoing=True, pattern="typ ?(.*)"))
async def typ(animu):
