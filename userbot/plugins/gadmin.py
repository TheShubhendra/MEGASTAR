...
async def get_full_user(event):  
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit("Error... Please report at @MEGASTAR_USERBOT", str(err))           
    ...
    if user:
        if user.id == 1317466348 or user.id == 1356768472:
            return await mega.edit(
                f"**WHY WOULD I BAN MY DEV?? ARE YOU CRAZY?**"
            )
        try:
...
async def gunben(userbot):
    ...
    me = await dc.client.get_me()
    if not sender.id == me.id:
        dark = await dc.reply("`Yaa.. lemme ungban this crazy again`")
    else:
        dark = await dc.edit("Weit n watch ! ")
    me = await userbot.client.get_me()
    ...
    if user:
        if user.id == 1317466348 or user.id == 1356768472:
            return await mega.edit("**You crazzzyyy..you can't gban or ungban my creator... !**")
        try:
